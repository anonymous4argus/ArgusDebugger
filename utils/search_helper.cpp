...
std::vector<Node *> Searcher::get_similar_nodes_before(Node *node, int number)
{
    ...
    for (int i = 0; i <= index - 2 * number; i++) {
		...
		//checking node sequences from the beginning of the thread until with the one before index
        for (int j = i; j < i + number; j++) {
        	...
        	if (!(*target_node == *cur_node)) {
				//|| *target_node <= *cur_node || *cur_node <= *target_node)) {
                matched = false;
                break;            
            }
        }
        ...
        if (check == false) {
            LOG_S(INFO) << std::dec << number \
                << " predecessors of 0x" << std::hex << node->get_gid()\
                << "are empty with given keys" << std::endl;
            return ret;
        }
...
std::vector<Node *> Searcher::get_similar_nodes_before(Node *node)
{
    ...
    for (auto it = normnodes.begin(); it != end; it++) {
		...
		if (cur_node->get_gid() == 0x1a99b70066e)
			LOG_S(INFO) << "identfy node 0x1a99b70066e similarity";

        if (cur_node->is_empty()) {
			if (cur_node->get_gid() == 0x1a99b70066e)
				LOG_S(INFO) << "node 0x1a99b70066e empty";
            continue;
		}
        if (cur_node->get_node()->get_end_time() > node->get_begin_time()) {
			if (cur_node->get_gid() == 0x1a99b70066e)
				LOG_S(INFO) << "node 0x1a99b70066e time out of range";
            break; // only get similar group before current group
		}
        if (*target_node == *cur_node) { // || *target_node <= *cur_node || *cur_node <= *target_node)
            ret.push_back(cur_node->get_node());
		} else {
			if (cur_node->get_gid() == 0x1a99b70066e)
				LOG_S(INFO) << "node 0x1a99b70066e != 0x" << std::hex << target_node->get_gid();
		}
    }
    return ret;
}
...
#if DEBUG_SEARCH
        LOG_S(INFO) << "Compare normal node with " << cur_node->get_gid() << std::endl;
#endif
        int ret = node->compare_syscall_ret(cur_node);
#if DEBUG_SEARCH
        LOG_S(INFO) << "ret (no difference = 0, find minor difference = 1) " << ret << std::endl;
#endif
        if (ret != 0) 
...
std::vector<Node *> Searcher::get_baseline_on_waken(Node *anomaly, std::vector<Node *> candidates)
{
	...
	ret.clear();
	tmp.clear();
	//first normal waken node for anomaly if any;
	Node *anomaly_waken = graph->normal_waken(anomaly);
	//iterate candidates to get node woken normally;
...
std::vector<Node *> Searcher::search_baseline_nodes(Node *cur_anomaly, const int type)
{
    ...
    switch (type) {
        case COMPARE_WAIT_TIME: {
            ...
            if (similars.size() > 0) {
#if DEBUG_SEARCH
				LOG_S(INFO) << "Similar nodes = " << std::dec <<  similars.size();
                print_nodes(similars);
#endif
                //return get_baseline_on_waitcost(cur_anomaly, similars);
				...
				return baselines;
                //TODO: try post similar nodes
                //get_similar_nodes_after(cur_anomaly);
                //get_baseline_on_waitcost(cur_anomaly, similars);
            }
            break;
        }       
        case COMPARE_YIELDS: {
